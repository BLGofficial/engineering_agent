...
function Spinner() {
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      <p className="text-amber-400 font-mono text-xs tracking-widest animate-pulse">
        ENGINEERING ROLE…
      </p>
    </div>
  );
...
function CodeBlock({ content, style }: { content: string; style?: React.CSSProperties }) {
  return (
    <pre
      style={style}
      className="bg-slate-950 border border-slate-700/50 rounded-lg p-5 font-mono text-xs leading-relaxed text-slate-200 whitespace-pre-wrap break-words overflow-y-auto max-h-96 shadow-xl hover:border-slate-600/50 transition-colors"
    >
      {content}
    </pre>
  );
  ...
  return (
    <div className="flex flex-col gap-5">
      {[data.example1, data.example2].map((ex, i) => (
        ...
        >
          <div className="bg-gradient-to-r from-slate-900 to-slate-800 px-5 py-3 border-b border-slate-700/50">
            <span className="text-amber-400 font-mono text-xs tracking-widest flex items-center gap-2">
              <Sparkles size={12} />
          ...
          <div className="p-5 flex flex-col gap-3">
            <div>
              ...
              <div className="text-slate-100 text-sm leading-relaxed">{ex.input}</div>
            </div>
            <div className="border-t border-slate-700/30 pt-3">
              <div className="text-blue-400/70 text-xs tracking-widest font-mono mb-2">OUTPUT</div>
              <div className="text-slate-200 text-sm leading-relaxed">{ex.output}</div>
            </div>
          </div>
        </div>
      ))}
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-200 font-sans flex flex-col items-center p-4 sm:p-8 pb-16 relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute bottom-1/3 right-1/4 w-80 h-80 bg-blue-500/5 rounded-full blur-3xl animate-pulse" style={{ animationDuration: "10s", animationDelay: "2s" }} />
      </div>
      ...
      {/* Header */}
      <div className="text-center mb-12 max-w-3xl relative z-10 animate-slideUp">
        <div className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-500/10 to-blue-500/10 border border-amber-500/30 rounded-full px-4 py-2 mb-6 backdrop-blur-sm hover:border-amber-500/50 transition-all duration-300">
          <div className="w-2 h-2 rounded-full bg-amber-400 animate-pulseGlow" />
          <span className="text-amber-400 font-mono text-xs tracking-widest">
            SYSTEM ACTIVE
          </span>
        </div>
        <h1 className="text-5xl sm:text-6xl font-bold bg-gradient-to-r from-slate-50 via-amber-200 to-slate-50 bg-clip-text text-transparent mb-3 tracking-tight">
      ...
      {/* Input Card */}
      <div className="w-full max-w-2xl bg-gradient-to-br from-slate-800/40 to-slate-900/40 border border-slate-700/50 rounded-2xl p-7 mb-7 shadow-2xl backdrop-blur-sm hover:border-slate-600/50 transition-all duration-300 relative z-10 animate-fadeIn">
        <label className="block font-mono text-xs text-amber-400 tracking-widest mb-4 flex items-center gap-2">
          <Sparkles size={14} />
...
• A bilingual customer support agent for a fintech startup targeting Eastern Europe`}
          className="w-full min-h-40 bg-slate-950/60 border border-slate-700/50 rounded-lg text-slate-100 text-sm leading-relaxed p-4 resize-vertical font-sans focus:border-amber-500/50 focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-all duration-300 placeholder-slate-500"
        />
        <div className="flex justify-between items-center mt-4">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-6 py-3 rounded-lg font-mono text-sm font-bold tracking-wider transition-all duration-300 ${
              loading || !input.trim()
      ...
      {/* Error */}
      {error && (
        <div className="w-full max-w-2xl bg-gradient-to-r from-red-950/40 to-rose-950/40 border border-red-700/50 rounded-lg p-4 text-red-300 text-sm mb-6 backdrop-blur-sm animate-slideUp relative z-10">
          ⚠ {error}
        </div>
      )}
      ...
      {/* Loading */}
      {loading && (
        <div className="w-full max-w-2xl bg-gradient-to-br from-slate-800/40 to-slate-900/40 border border-slate-700/50 rounded-xl p-12 flex justify-center mb-6 backdrop-blur-sm relative z-10 animate-fadeIn">
          <Spinner />
        </div>
      )}
      ...
      {/* Results */}
      {roleData && (
        <div className="w-full max-w-2xl animate-slideUp relative z-10">
          {/* Role Header Card */}
          <div className="bg-gradient-to-br from-slate-800/60 to-slate-900/60 border border-slate-700/50 rounded-2xl p-7 mb-6 shadow-2xl backdrop-blur-sm hover:border-slate-600/50 transition-all duration-300">
            <div className="flex justify-between items-start flex-wrap gap-4 mb-6">
              ...
              <div className="flex gap-2 flex-wrap">
                <span className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/50 rounded-full px-3 py-1 text-green-300 text-xs font-mono">
                  {roleData.keyAttributes.expertiseLevel}
                </span>
                <span className="bg-gradient-to-r from-purple-500/20 to-pink-500/20 border border-purple-500/50 rounded-full px-3 py-1 text-purple-300 text-xs font-mono">
                  {roleData.keyAttributes.domainFocus}
                </span>
                <span className="bg-gradient-to-r from-orange-500/20 to-amber-500/20 border border-orange-500/50 rounded-full px-3 py-1 text-orange-300 text-xs font-mono">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                <div className="text-amber-400/70 text-xs tracking-widest font-mono mb-3 flex items-center gap-2">
                  <span className="text-amber-400">→</span> USE FOR
                </div>
                {roleData.primaryUseCases.slice(0, 3).map((u, i) => (
                  <div key={i} className="text-slate-300 text-sm mb-2 hover:text-amber-300 transition-colors">
                    • {u}
                  </div>
                ))}
              </div>
              <div>
                <div className="text-red-400/70 text-xs tracking-widest font-mono mb-3 flex items-center gap-2">
                  <span className="text-red-400">✕</span> AVOID FOR
                </div>
                {roleData.antiUseCases.map((u, i) => (
                  <div key={i} className="text-slate-400 text-sm mb-2 hover:text-red-300 transition-colors">
                    • {u}
                  </div>
                ))}
              </div>
          ...
          {/* Tabs */}
          <div className="bg-gradient-to-br from-slate-800/40 to-slate-900/40 border border-slate-700/50 rounded-2xl overflow-hidden mb-6 backdrop-blur-sm shadow-xl">
            <div className="flex overflow-x-auto border-b border-slate-700/50 bg-slate-950/30">
              {TABS.map((tab) => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={`px-4 py-3 font-mono text-xs tracking-widest whitespace-nowrap transition-all duration-300 border-b-2 ${
                    activeTab === tab
          ...
          <div className="flex gap-3 justify-end items-center">
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 rounded-lg border border-slate-700/50 text-slate-400 text-sm font-mono tracking-wider hover:border-slate-600/50 hover:text-slate-300 hover:bg-slate-800/20 transition-all duration-300"
            >
              <RotateCcw size={14} />
              NEW ROLE
            </button>
            <button
              ...
              className={`flex items-center gap-2 px-6 py-2.5 rounded-lg font-mono text-sm font-bold tracking-wider transition-all duration-300 ${
                downloading
                  ? "bg-slate-800 text-slate-600 cursor-not-allowed"
